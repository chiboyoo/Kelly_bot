...
async def auto_loop(app):
    while True:
        ...
        if sig=="BUY" and not pos:
            price=info["price"]
            # Open LONG
            positions[pair_label]={"price":price,"alloc":s["alloc"],"time":time.time(),"high":price,"low":price,"rsi":info["rsi"],"side":"LONG","lev":s.get("leverage",10)}
            ...
            save_state(s)
            try: await app.bot.send_message(chat_id=MY_ID, text=f"🟢 LONG {pair_label} @ {format_price(price)} RSI {info['rsi']:.0f} SMA10 {format_price(info['sma10'])}>{format_price(info['sma30'])} x{s.get('leverage',10)}")
            except: pass
        elif pos:
...
async def cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="pair":
        ...
        s["mode"]="LIVE" if s["mode"]=="DEMO" else "DEMO"
        if s["mode"]=="LIVE":
            bal,err=await get_futures_balance()
            s["real_bal"]=bal if bal is not None else 0.0
        save_state(s)
        mode_txt=f"🔴 LIVE ${s.get('real_bal',0):.2f}" if s["mode"]=="LIVE" else f"🧪 DEMO ${s['demo_bal']:.2f}"
        await q.edit_message_text(f"Switched to {mode_txt}", reply_markup=kb(s))
    elif data=="auto":
        ...
        await q.edit_message_text("🔄 Checking real positions...")
        live_pos,perr=await get_futures_positions_live()
        if perr: await q.edit_message_text(f"❌ {perr}", reply_markup=kb(s))
        elif not live_pos: await q.edit_message_text("No real positions on Bitget", reply_markup=kb(s))
        else:
            txt="Closing REAL:\n\n"
            for p in live_pos:
                try:
            ...
            bal,_=await get_futures_balance()
            if bal is not None: s["real_bal"]=bal; s["auto"]=False; s["positions"]={}; save_state(s)
